...
angular
  .module('directive.vendor.draganddrop', [])
      ...
      domElement.addEventListener('dragstart', function(e) {
        ...
        $(document).mousemove(function(event) {
          ...
          $(e.currentTarget).offset({
            top: currentMousePos.x,
            left: currentMousePos.y
          });
        });
...
function dropDirective($parse) {
  return {
    ...
    link: function (scope, element, attrs) {
      ...
      domElement.addEventListener('mouseover', mouseOverListener);
      domElement.addEventListener('mouseleave', mouseOutListener);
      ...
      function dropListener(event) {
        ...
        scope.$apply(function() {
          ...
          switch(data['json/card-component'].widget) {
            case 'Image':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-file-image-o"></i>');
              break;

            case 'Text':
              el = angular.element('<i id="icon-paragraph" class="' +
                'canvas-item fa fa-paragraph"></i>');
              break;

            case 'Square':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-square"></i>');
              break;

            case 'Circle':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-circle"></i>');
              break;

            case 'Icon':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-archive"></i>');
              break;

            case 'Save':
              el = angular.element('<i id="icon-paragraph" class="' +
                'canvas-item fa fa-save"></i>');
              break;

            case 'Erase':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-eraser"></i>');
              break;

            case 'Rotate':
              el = angular.element('<i id="icon-image" class="' +
                'canvas-item fa fa-rotate-right"></i>');
              break;
